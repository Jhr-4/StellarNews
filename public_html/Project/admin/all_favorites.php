...
<?php
//POST DATA FROM UNFAVORITE FOR SPECIFC ARTICLE:
if (isset($_POST["users"]) && isset($_POST["article"])) {
    $user_ids = $_POST["users"]; //se() doesn't like arrays so we'll just do this
    $article_id = $_POST["article"]; //se() doesn't like arrays so we'll just do this
    if (empty($user_ids) || empty($article_id)) {
        flash("Both article and users need to be selected", "warning");
    } else {
        $db = getDB();
        $stmt = $db->prepare("INSERT INTO `UserArticles` (user_id, article_id, is_active) VALUES (:uid, :aid, 1) 
...
$form = [
    ["type" => "input", "id" => "username", "name" => "username", "label" => "Username", "placeholder" => "Username", "include_margin" => false],
...
$query = "SELECT ArticlesTable.id, title, site_url, image_url, news_text, news_summary_long, ArticlesTable.is_active, UserArticles.user_id, UserArticles.is_active AS userArticle_isActive, Users.username, UserArticles.user_id 
FROM  `ArticlesTable` LEFT JOIN `UserArticles` on ArticlesTable.id = UserArticles.article_id
LEFT JOIN Users on Users.id = UserArticles.user_id WHERE UserArticles.is_active";
...
$article_titles = []; //to find all the usernames associated with the article id and unsetting duplicate articles
for ($i = 0; $i < count($results); $i++) { //adds data for null values (that were added by manual create)
    foreach ($results[$i] as $k => $v) {
        ...
        if ($k === 'title') {
            if (!isset($article_titles[$v])) { //if name of article isn't set
                $article_titles[$v][] = ["username" => $results[$i]["username"], "user_id" => $results[$i]["user_id"]]; //add username/id for title
            } else {
                $article_titles[$v][] = ["username" => $results[$i]["username"], "user_id" => $results[$i]["user_id"]];
                unset($results[$i]);
            }
        }
    }
...
<div class="container-fluid">
    ...
    <div class="container-fluid mb-3">
        <h1 class="col-lg-auto col-md-auto col-sm-auto">All Favorited Articles </h1>
    </div>
            ...
            <div class="col d-flex">
                <div class="card bg-dark text-white border-white flex-fill mt-3">
                    ...
                    <div class="card-img-overlay d-flex flex-column">
                        ...
                        <input id="article_<?php se($article['id'], 'id'); ?>" type="hidden" name="article" value="<?php se($article['id'], 'id'); ?>" />

                        <div class="text-center mt-auto">
                            <!--CLICK TO UNFAVORITE ALL, LINK = ?TOGGEL_ARTICLE_ID-->
                            <button type="submit" class="btn btn-danger border-light" data-toggle="tooltip" data-placement="top" title="Unfavorite Selected">
